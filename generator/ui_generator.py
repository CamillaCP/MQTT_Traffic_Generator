    ...
    if mode_var.get() == "Manual Configuration":
        ...
        if role_var.get():
            ...
            if role_var.get().lower() == "publisher":
                ...
                # Show DeviceType dropdown for Publisher role
                device_type_frame.grid(row=6, column=0, columnspan=2, pady=10, sticky='ew')

                # If the DeviceType is Counterfeit, show the Hidden Message and Embedding Method fields
...
role_frame = create_labeled_combobox(main_frame, "Role", role_var, ["Publisher", "Subscriber"])  # Capitalized for UI display
...
type_frame = create_labeled_combobox(main_frame, "Device Timing", type_var, ["Periodic", "Event"])  # Capitalized for UI display
...
distribution_frame = create_labeled_combobox(main_frame, "Distribution", distribution_var, ["Uniform", "Exponential", "Normal"])  # Capitalized for UI display
device_type_frame = create_labeled_combobox(main_frame, "Device Type", device_type_var, ["Legit", "Counterfeit"])  # Capitalized for UI display
hidden_message_frame = create_labeled_entry(main_frame, "Hidden Message", hidden_message_var)
embedding_method_frame = create_labeled_combobox(main_frame, "Embedding Method", embedding_method_var, ["Case", "ID"])  # Capitalized for UI display
